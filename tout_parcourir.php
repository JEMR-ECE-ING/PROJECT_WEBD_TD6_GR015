...
// Récupérer la catégorie demandée (si aucune, on affiche la page générale)
...
// Récupérer les activités + coachs par catégorie
function getActivitesEtCoachs(PDO $pdo, int $idCategorie): array {
    // On récupère les activités de cette catégorie
    $stmt = $pdo->prepare("
        SELECT 
            a.id_activite, a.nom_activite, a.description AS desc_activite, a.prix, a.duree_seance,
            c.id_coach, c.telephone_bureau, c.photo_url, c.video_url, c.statut_disponibilite, c.bureau,
            u.nom AS nom_coach, u.prenom AS prenom_coach, u.email AS email_coach
        ...
        <section class="services-section">
            ...
            <div class="services-grid">
                ...
                <div class="service-card" onclick="showCategory('sports_competition')">
                    <div><img src="images/trophe.png" class="service-icon"></div>
                    <h3>Sports de Compétition</h3>
                    <p>Basketball, Football, Rugby, Tennis, Natation, Plongeon</p>
                    <button class="cta-button">Voir plus</button>
                </div>

                <div class="service-card" onclick="alert('Affichage spécifique à la salle de sport en développement')">
                    <div><img src="images/omnes.png" class="service-icon"></div>
    ...
    ?>
        <section class="services-section">
            <h2 class="services-title">
                <?= $categorie === 'activites_sportives' ? 'Activités Sportives' : 'Sports de Compétition' ?>
            </h2>
            <div class="services-grid">
                ...
                <div class="service-card">
                    ...
                    <p><strong>Prix :</strong> <?= number_format(num: $act['prix'], ) ?> €</p>
                    <p><strong>Durée :</strong> <?= intval($act['duree_seance']) ?> min</p>
                    <hr style="border-color:#00ff88; margin:1rem 0;">
                    <h4>Coach associé :</h4>
                    <?php if ($act['id_coach']): ?>
                        <p><strong>Nom :</strong> <?= htmlspecialchars($act['nom_coach'] . ' ' . $act['prenom_coach']) ?></p>
                        <p><strong>Email :</strong> <?= htmlspecialchars($act['email_coach']) ?></p>
                        <p><strong>Téléphone :</strong> <?= htmlspecialchars($act['telephone_bureau']) ?></p>
                        <p><strong>Salle / Bureau :</strong> <?= htmlspecialchars($act['bureau']) ?></p>
                    <?php else: ?>
